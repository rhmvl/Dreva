...
deps: $(EXTERNAL_DIR)
	...
	@if [ ! -d $(EXTERNAL_DIR)/raylib ]; then \
		echo "Installing Raylib"; \
		tmp=$$(mktemp); \
		$(DOWNLOAD) "$(RAYLIB_URL)" > "$$tmp"; \
		mkdir -p $(EXTERNAL_DIR)/raylib; \
		tar -xzf "$$tmp" -C $(EXTERNAL_DIR)/raylib --strip-components=1; \
		rm -f "$$tmp"; \
	else echo "Raylib already present"; fi
	...
	@if [ ! -f $(EXTERNAL_DIR)/entt/entt.hpp ]; then \
	...
	@if [ ! -d $(EXTERNAL_DIR)/rlimgui ]; then \
		echo "Installing rlImGui"; \
		git clone --depth 1 $(RLIMGUI_REPO) $(EXTERNAL_DIR)/rlimgui; \
	else echo "rlImGui already present"; fi
